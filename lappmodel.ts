...
export class LAppModel extends CubismUserModel {
  ...
  /**
   * model3.jsonからモデルを生成する。從model3.json生成模型。
   * model3.jsonの記述に従ってモデル生成、モーション、物理演算などのコンポーネント生成を行う。
   * 根據model3.json中的描述生成模型，運動和物理計算等組件。
   *
    ...
    const setupBreath = (): void => {
      ...
      breathParameters.pushBack(
        new BreathParameterData(
          ...
          0.8,     //offset
          0.8,     //peak
          3.2345,     //cycle 確實是來回一次的時間。可是例如用4就會在呼吸循環一次後停頓。
          0.5      //weight 超過0.5就會有點像鞭打的效應，會不會是ease-in, ease-out的弧度？
        )
      );
